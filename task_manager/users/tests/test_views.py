...
class UsersViewsTest(TestCase):
    ...
    def test_user_update_view_other_profile(self):
        self.client.login(username='testuser', password=TEST_PASSWORD)
        response = self.client.get(reverse('user_update', args=[self.other_user.id]))
    ...
    def test_user_delete_view_other_user(self):
        self.client.login(username='testuser', password=TEST_PASSWORD)
        response = self.client.get(reverse('user_delete', args=[self.other_user.id]))
        self.assertEqual(response.status_code, 302)  # Redirect to users list
        self.assertRedirects(response, reverse('users'))
...
class AuthenticationViewsTest(TestCase):
    ...
    def test_login_view_post_invalid(self):
        ...
        self.assertContains(response, 'Please enter correct username and password')
