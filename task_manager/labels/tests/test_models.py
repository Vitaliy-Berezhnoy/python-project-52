...
class LabelModelTest(TestCase):
    ...
    def test_create_label(self):
        """ метка создается с правильными атрибутами"""
    ...
    def test_label_string_representation(self):
        """строковое представление метки возвращает её имя"""
        label = Label.objects.create(**self.label_data)
    ...
    def test_label_ordering(self):
        ...
        self.assertEqual(all_labels[0].name, "bug")
        self.assertEqual(all_labels[1].name, "feature")
        self.assertEqual(all_labels[2].name, "urgent")
...
class LabelTaskRelationshipTest(TestCase):
    ...
    def test_task_can_have_multiple_labels(self):
        """ задача может иметь несколько меток"""
    ...
    def test_label_can_be_used_in_multiple_tasks(self):
        """ метка может использоваться в нескольких задачах"""
        task1 = Task.objects.create(
            name="Задача 1", status=self.status, author=self.user
        )
        task2 = Task.objects.create(
            name="Задача 2", status=self.status, author=self.user
        )
    ...
    def test_task_without_labels(self):
        """ задача может быть создана без меток"""
        task = Task.objects.create(
            name="Задача без меток", status=self.status, author=self.user
        )
        self.assertEqual(task.labels.count(), 0)

    def test_label_related_name(self):
        """ related_name работает корректно"""
        task = Task.objects.create(
            name="Тестовая задача", status=self.status, author=self.user
        )
